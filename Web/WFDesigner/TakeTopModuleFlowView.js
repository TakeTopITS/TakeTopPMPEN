(function (b) {
    ...
    a.config = {
        ...
        props: {
            attr: {
                top: 10,
                left: parent.window.innerWidth - 440
            },
            props: {}
        },
        restore: "",
    ...
    a.editors = {
        ...
        textTaskEditor: function () {
            ...
            this.init = function (i, h, m, l, j) {
                ...
                g = l;
                f = j;
                var $txt = b('<input  style="width:100%;" />');
                ////将text属性中的url属性赋值给input的data-url属性               
                if (i[e].url) {
                    $txt.data("url", i[e].url);
                }
                $txt.val(g.text()).change(function () {
                    i[e].value = b(this).val();
                    ////读取绑定在控件上的url属性
                    i[e].url = b(this).data("url");
                    b(f).trigger("textchange", [b(this).val(), g])
                }).appendTo("#" + c);

                ////添加选择按钮 
                b('<input type="button" value="选择(Select)" onclick="showModuleTree(this)" />').appendTo('#' + c);

                b("#" + c).data("editor", this)
            };
            this.destroy = function () {
                b("#" + c + " input[type!=button]").each(function () {
                    d[e].value = b(this).val();
                    ////读取绑定在控件上的url属性
                    d[e].url = b(this).data("url");
                    b(f).trigger("textchange", [b(this).val(), g])
                })
            }
        }
    ...
    a.init = function (x, r) {
        ...
        var w = function (c, i) {
            ...
            if (i.getId().substring(0, 4) == "rect") {
                ...
                if (json.type == "task") {
                    ...
                    openToTargetFrame(openToTargetFrame);
                }
                q[i.getId()] = null;
                i.remove()
            } else {
        ...
        b(y).bind("addrect",
            function (j, c, k) {
                if (a.config.tools.states[c].props.guid) {
...
function openToTargetFrame(url) {
    ...
    if (url.indexOf("layui.com") < 0 & url !== "") {
        top.frames[0].frames[2].parent.frames["rightTabFrame"].popShowByURL(url, url.replace('.aspx', '').replace("TT",''),'99%','99%',window.location);
    }
