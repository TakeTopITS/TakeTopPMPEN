...
public class Msg
{
    ...
    public bool SendRTXMsgByUserRTXCode(string strUserRTXCode, string strMsg)
    {
        ...
        for (int i = 0; i < lst.Count; i++)
        {
            ...
            try
            {
                RootObj.SendNotify(strUserRTXCode, LanguageHandle.GetWord("SystemMessage").ToString().Trim(), 60000000, strMsg); //获取版本信息
                return true;
            }
            catch
            {
    ...
    public bool SendMail(string strUserCode, string strSubject, string strBody, string strSendUserCode)
    {
        ...
        ///添加邮件主题
        mailMsg.Subject = strSubject;
        mailMsg.Subject = mailMsg.Subject.Replace("审批通知", LanguageHandle.GetWord("ZZShengPiTongZhi").ToString().Trim());  
