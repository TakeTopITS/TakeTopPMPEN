...
public partial class TTRiskToTask : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //this.Title = "风险：" + strRiskID + " " + projectRisk.Risk + " 转成项目任务";
        ...
        if (Page.IsPostBack != true)
        {
            ...
            TB_Task.Text = projectRisk.Risk.Trim();
            DLC_BeginDate.Text = projectRisk.EffectDate.ToString("yyyy-MM-dd");
            DLC_EndDate.Text = projectRisk.EffectDate.ToString("yyyy-MM-dd");
            LB_MakeDate.Text = DateTime.Now.ToShortDateString();

            if (strIsMobileDevice == "YES")
            {
                HT_Operation.Text = LanguageHandle.GetWord("FengXianNeiRong").ToString().Trim() + projectRisk.Detail.Trim();
            }
            else
            {
                HE_Operation.Text = LanguageHandle.GetWord("FengXianNeiRong").ToString().Trim() + projectRisk.Detail.Trim();
            }
    ...
    protected void AddTask()
    {
        ...
        else
        {
            ...
            projectTask.RiskID = int.Parse(LB_RiskID.Text.Trim());
            ...
            try
            {
                ...
                UpdateRiskStatus(LB_RiskID.Text.Trim());
            }
            catch
            {
    ...
    {
        string strProjectID = LB_ProjectID.Text.Trim();
        string strRiskID = LB_RiskID.Text.Trim();

        string strHQL = "from ProjectTask as projectTask where projectTask.ProjectID = " + strProjectID + " and projectTask.RiskID = " + strRiskID + " Order by projectTask.TaskID ASC";
        ProjectTask projectTask = new ProjectTask();
        ProjectTaskBLL projectTaskBLL = new ProjectTaskBLL();
        IList lst = projectTaskBLL.GetAllProjectTasks(strHQL);
        DataGrid1.DataSource = lst;
    ...
    protected void UpdateRiskStatus(string strRiskID)
    {
