...
Ext.define("Gnt.locale.En", {
    ...
    l10n: {
        ...
        "Gnt.widget.taskeditor.TaskForm": {
            ...
            leaderCodeText: "LeaderCode",
            leaderText: "Leader Name",
            workhourText: "Workhour",
            actualhourText: "Actualhour",
            budgetText: "Budget",
            expenseText: "Expense",
...
Ext.define("Gnt.model.Task", {
    ...
    nameField: "Name",
    leaderCodeField: "LeaderCode",
    leaderField: "Leader",

    workhourField: "Workhour",
    actualhourField: "Actualhour",
    budgetField: "Budget",
    expenseField: "Expense",

    requireNumberField: "RequireNumber",
    finishedNumberField: "FinishedNumber",
    priceField: "Price",
    ...
    //基线开始函数，需要修改
    setBaselineStartDate: function (a, g, f) {
        var e,
            d;
        this.beginEdit();
        if (!a) {
            //this.set(this.durationField, null);
            this.set(this.baselineStartDateField, null)
        } else {
            var c = this.getCalendar();
            if (f && !this.isManuallyScheduled()) {
                a = this.skipNonWorkingTime(a, !this.isMilestone())
            ...
            var b = this.getSchedulingMode();
            this.set(this.baselineStartDateField, a);
            //if (g !== false) {
            //    if (b == "EffortDriven") {
            //        this.set(this.endDateField, this.calculateEffortDrivenEndDate(a, this.getEffort()))
            //    } else {
    ...
    //基线 结束函数，需要修改
    setBaselineEndDate: function (d, h, g) {
        var f,
            a;
        this.beginEdit();
        if (!d) {
            //this.set(this.durationField, null);
            this.set(this.baselineEndDateField, null)
        } else {
            var c = this.getCalendar();
            if (g && !this.isManuallyScheduled()) {
                d = this.skipNonWorkingTime(d, !this.isMilestone())
            }
            var b = this.getSchedulingMode();
            this.set(this.baselineEndDateField, d);

            //var e = this.getCalendar();
            //a = this.getBaselineStartDate();
            //if (d < a) {
            //    d = a
            //}
            //if (g && !this.isManuallyScheduled()) {
            ...
            //    f = this.getDuration();
            //    if (Ext.isNumber(f)) {
            //        this.set(this.baselineStartDateField, this.calculateStartDate(d, f, this.getDurationUnit()));
            //        this.set(this.baselineEndDateField, d)
            //    } else {
            //        this.set(this.baselineEndDateField, d)
            //    }
    ...
    //----Jack add Code 20180601  BEGIN--任务编辑窗口真正起作用的-----
    calculateWeekendDays: function () {

        var varWeekendSecondDaySeven, varWeekendDays;

        if (varWeekendSecondDay == 0) {

            varWeekendSecondDaySeven = 7;

        }
        else {

            varWeekendSecondDaySeven = varWeekendSecondDay;
        }

        if (varWeekendsAreWorkdays) {

            varWeekendDays = 0;
        }
        else {

            varWeekendDays = 0 - (varWeekendSecondDaySeven - varWeekendFirstDay + 1);
        }

        return varWeekendDays;
    },
    ...
    getDisplayBaselineStartDate: function (g, d, e, b, c) {
        var f = this.getBaselineEndDate(),
            a = this.getBaselineStartDate();
        if (arguments.length < 3) {
            e = this.getBaselineStartDate();
            if (arguments.length < 2) {
                d = true
            }
        }
    ...
    getDisplayBaselineEndDate: function (e, c, d, a, b) {
        if (arguments.length < 3) {
            d = this.getBaselineEndDate();
            if (arguments.length < 2) {
                c = true
            }
        }
...
Ext.define("Gnt.data.Calendar", {
    ...
    daysPerMonth: 30,
    daysPerWeek: varWeekendEndDay - varWeekendFirstDay + 1,
    hoursPerDay: 24,
    unitsInMs: null,
    defaultNonWorkingTimeCssCls: "gnt-holiday",

    weekendsAreWorkdays: varWeekendsAreWorkdays,
    weekendFirstDay: varWeekendFirstDay,
    weekendSecondDay: varWeekendSecondDay,
    ...
    calculateEndDate: function (a, f, b) {
        ...
        //f = this.convertDurationToMs(f, b) + this.calculateWeekendDays() * 24 * 60 * 60 * 1000;
    ...
    calculateStartDate: function (d, c, b) {
        ...
        //c = this.convertDurationToMs(c, b) + this.calculateWeekendDays() * 24 * 60 * 60 * 1000;
...
Ext.define("Gnt.plugin.TaskContextMenu", {
   ...
   /*    JackZhongCode;*/
        ...
        if (varUserIsCanUpdatePlan === "False" || varProjectStatus === "Suspended" || varProjectStatus === "Cancel" || varProjectStatus === "Acceptance" || varProjectStatus === "CaseClosed" || varProjectStatus === "Archived" || varBusinessType === "COMPARE") {
            ...
            var that = this;
            Ext.Ajax.request({
                url: 'Handler/GetPageModuleDataForGanttChartRightMemu.ashx?LangCode=' + varLangCode,
                method: 'GET',
                async: false,
                    ...
                    if (success) {
                        ...
                        if (responseJson) {
                            for (var i = 0; i < responseJson.length; i++) {
                             ...
                             /*   try {*/
                                    var obj = {
                                        handler: that.convertDynaLabel,
                                        requiresTask: true,
                                        itemId: label,
                                        pageName: item.pagename,
                                        text: that.L(item.modulelabel)
                                    };
                                    data.splice(data.length - 1, 0, obj);
                                //}
                                //catch (err) {
                                //    var txt = "此页面存在一个错误。\n\n";
    ...
    //动态菜单的统一方法
    convertDynaLabel: function (data) {

        ////刷新缓存
        if (this.rec.id.indexOf("ext-record") > 0) {

            alert(getRightClickPlanPopMsgByLangCode());
            location.reload();

        }
        else {

            popShowByURL(data.pageName + this.rec.id.replace('MyTaskModel-', '') + "&ProjectID=" + pid.toString().substr(0, pid.toString().length - 2), "Plan Work", 800, 600, window.location);
...
Ext.define("Gnt.field.BaselineEndDate", {
    ...
    constructor: function (a) {
        ...
        Ext.apply(this, a);
        if (a.task && !a.value) {
            a.value = a.task.getBaselineEndDate()
        }
        this.setSuppressTaskUpdate(true);
        this.callParent([a]);
        this.setSuppressTaskUpdate(false);
    ...
    onSetTask: function () {
        this.setValue(this.task.getBaselineEndDate())
    },
    rawToValue: function (a) {
        if (!a) {
            return null
    ...
    valueToVisible: function (b, a) {
        a = a || this.task;
        return a.getDisplayBaselineEndDate(this.format, this.adjustMilestones, b, true)
    },
    visibleToValue: function (a) {
        if (a && this.task) {
            if (!Ext.Date.formatContainsHourInfo(this.format) && a - Ext.Date.clearTime(a, true) === 0) {
    ...
    getErrors: function (a) {
        ...
        if (this.validateStartDate) {
            a = this.rawToValue(a);
            if (this.task && a) {
                if (a < this.task.getBaselineStartDate()) {
                    return [this.L("endBeforeStartText")]
                }
            }
        }
    ...
    applyChanges: function (a) {
        ...
        var b = a.getTaskStore(true) || this.taskStore;
        if (this.value) {
            a.setBaselineEndDate(this.value, this.keepDuration, b.skipWeekendsDuringDragDrop)
        } else {
            a.setBaselineEndDate(null)
        }
    },
    setVisibleValue: function (a) {
        this.setValue(this.rawToValue(Ext.Date.format(a, this.format)))
    ...
    setValue: function (a) {
        ...
        if (this.instantUpdate && !this.getSuppressTaskUpdate() && this.task) {
            this.applyChanges();
            var b = this.task.getBaselineEndDate();
            if (b - this.getValue() !== 0) {
                this.callParent([b])
            }
            this.task.fireEvent("taskupdated", this.task, this)
...
Ext.define("Gnt.field.BaselineStartDate", {
    ...
    constructor: function (a) {
        a = a || {};
        if (a.task && !a.value) {
            a.value = a.task.getBaselineStartDate()
        }
        this.setSuppressTaskUpdate(true);
        this.callParent([a]);
        this.setSuppressTaskUpdate(false);
    ...
    onSetTask: function () {
        this.setValue(this.task.getBaselineStartDate())
    },
    rawToValue: function (a) {
        if (!a) {
            return null
    ...
    valueToVisible: function (b, a) {
        a = a || this.task;
        return a.getDisplayBaselineStartDate(this.format, this.adjustMilestones, b, true)
    },
    visibleToValue: function (c) {
        var b = this.task;
        if (b && c) {
            var d = b.getBaselineEndDate();
            var a = !this.lastValue || this.lastValue - Ext.Date.clearTime(this.lastValue, true) === 0;
            if (this.adjustMilestones && b.isMilestone() && c - Ext.Date.clearTime(c, true) === 0 && a) {
                c = b.getCalendar().getCalendarDay(c).getAvailabilityEndFor(c) || c
            }
    ...
    applyChanges: function (a) {
        a = a || this.task;
        var b = a.getTaskStore(true) || this.taskStore;
        //a.setBaselineStartDate(this.value, this.keepDuration, b.skipWeekendsDuringDragDrop);
        a.setBaselineStartDate(this.value, b.skipWeekendsDuringDragDrop);
    },
    setVisibleValue: function (a) {
        this.setValue(this.rawToValue(Ext.Date.format(a, this.format)))
    },
    ...
    setValue: function (b) {
        ...
        if (this.instantUpdate && !this.getSuppressTaskUpdate() && this.task && this.task.taskStore && b) {
            this.applyChanges();
            var a = this.task.getBaselineStartDate();
            if (a - this.getValue() !== 0) {
                this.callParent([a])
            }
            this.task.fireEvent("taskupdated", this.task, this)
...
Ext.define("Gnt.widget.AssignmentEditGrid", {
        ...
        if (e) {
            ...
            if (!b) {
                ...
                if (this.confirmAddResource) {
                    ...
                    //取消新建资源，改为提示找不到资源，并重新输入
                    var g = Ext.String.htmlEncode(Ext.String.format(this.L("noResourceText"), e));
                    this.cellEditing.cancelEdit();
                    Ext.Msg.alert(this.L("noResourceText"), g);
                    //c()
                }
            } else {
                f.select(b, true)
    ...
    saveTaskAssignments: function () {
        ...
        //var ass = this.assignmentStore.data;
        //this.assignmentStore.data.each(
        //    function () {
        //        if( isNaN(this.data.ResourceId))
        //        {
        //            ass.remove(this);
        //        }
        //    }
...
Ext.define("Gnt.widget.taskeditor.TaskForm", {
    ...
    taskNameConfig: null,
    leaderCodeConfig: null,
    leaderConfig: null,

    workhourConfig: null,
    actualhourConfig: null,

    requireNumberConfig: null,
    finishedNumberConfig: null,
    priceConfig: null,
    unitNameConfig: null,
    ...
    constructor: function (b) {
        ...
        this.fieldNames = {
            ...
            leaderCodeField: a.leaderCodeField,
            leaderField: a.leaderField,

            workhourField: a.workhourField,
            actualhourField: a.actualhourField,

            requireNumberField: a.requireNumberField,
            finishedNumberField: a.finishedNumberField,
            priceField: a.priceField,
            unitNameField: a.unitNameField,

            budgetField: a.budgetField,
            expenseField: a.expenseField,

            priorityField: a.priorityField
        };
        Ext.apply(this, b, {
            border: false,
            layout: "anchor",
    ...
    buildFields: function () {
        var d = this,
            ...
            c = this.task,
            b = this.taskStore,
            i = '<table class="gnt-fieldcontainer-label-wrap"><td width="110" class="gnt-fieldcontainer-label">',
        ...
        this.items.push.call(this.items,
            ...
            },
            {
                ...
                items: [
                    a({
                        ...
                        //listeners: {
                        //    "select": function () {
                        //        alert(pid.toString().substr(0, pid.toString().length - 2));   //获取id为combo的值
                        //    }
                        //},
...

            {
                ...
                items: [
                    a({
                        xtype: 'combobox',
                        fieldLabel: this.L("priorityText"),
                        name: e.priorityField,
                        labelWidth: 60,
                        ...
                        editable: false,
                        emptyText: 'Normal',
                        value: g(e.priorityField),
                    }, this.priorityConfig),

                    a({
                        xtype: "textfield",
                        fieldLabel: this.L("workhourText"),
                        name: e.workhourField,
                        labelWidth: 60,
                        margins: "0 0 0 6",
                        width: 50,
                        flex: 1,
                        value: g(e.workhourField)
                    }, this.workhourConfig),

                    a({
                        xtype: "textfield",
                        fieldLabel: this.L("actualhourText"),
                        name: e.actualhourField,
                        labelWidth: 60,
                        margins: "0 0 0 6",
                        width: 50,
                        flex: 1,
                        value: g(e.actualhourField)
                    }, this.actualhourConfig)
                ]
            },

            {
                ...
                items: [
                    ...
                    a({
                        xtype: "textfield",
                        fieldLabel: this.L("budgetText"),
                        name: e.budgetField,
                        labelWidth: 60,
                        margins: "0 0 0 6",
                        width: 50,
                        flex: 1,
                        value: g(e.budgetField)
                    }, this.budgetConfig),

                    a({
                        xtype: "textfield",
                        fieldLabel: this.L("expenseText"),
                        name: e.expenseField,
                        labelWidth: 60,
                        margins: "0 0 0 6",
                        width: 50,
                        flex: 1,
                        value: g(e.expenseField)
                    }, this.expenseConfig)
...

            {
                ...
                items: [
                    a({
                        xtype: "textfield",
                        fieldLabel: this.L("requireNumberText"),
                        name: e.requireNumberField,
                        labelWidth: 60,
                        margins: "0 0 0 6",
                        width: 50,
                        flex: 1,
                        value: g(e.requireNumberField)
                    }, this.requireNumberConfig),

                    a({
                        xtype: "textfield",
                        fieldLabel: this.L("finishedNumberText"),
                        name: e.finishedNumberField,
                        labelWidth: 60,
                        margins: "0 0 0 6",
                        width: 50,
                        flex: 1,
                        value: g(e.finishedNumberField)
                    }, this.finishedNumberConfig),

                    a({
                        xtype: "textfield",
                        fieldLabel: this.L("priceText"),
                        name: e.priceField,
                        labelWidth: 60,
                        margins: "0 0 0 6",
                        width: 50,
                        flex: 1,
                        value: g(e.priceField)
                    }, this.priceConfig),

                    //a({
                    //    xtype: "textfield",
                    //    fieldLabel: this.L("unitNameText"),
                    //    name: e.unitNameField,
                    //    labelWidth: 60,
                    //    margins: "0 0 0 6",
                    //    width: 50,
                    //    flex: 1,
                    //    value: g(e.unitNameField)
                    //}, this.unitNameConfig)
...
Ext.define("Gnt.widget.taskeditor.TaskEditor", {
    ...
    constructor: function (b) {
        ...
        //if (this.showResourceGrid && this.resourceStore)
        //{
        //    this.assignmentGrid = Ext.create("Gnt.widget.ResourceEditGrid", Ext.apply({
        //        assignmentStore: this.assignmentStore,
        //        resourceStore: this.resourceStore,
        //        tbar: {
        //            layout: "auto",
        ...
        if (this.showNotes) {
            this.notesEditor = Ext.create(this.notesConfig || "Ext.form.field.HtmlEditor", Ext.applyIf(this.notesConfig, {
                items: [
                    {
                        xtype: "Ext.form.field.HtmlEditor",
                        value: this.task && this.task.noteField,
                        taskStore: this.taskStore,
                        task: this.task
                    }
                ],
...
Ext.define("Gnt.plugin.TaskEditor", {
    ...
    init: function (d) {
        ...
        var b = {
            ...
            e = ["l10n", "task", "taskStore", "assignmentStore", "generalText", "dependencyText", "addDependencyText", "dropDependencyText", "notesText", "advancedText", "wbsCodeText", "addAssignmentText", "dropAssignmentText", "showAssignmentGrid", "showDependencyGrid", "allowParentTaskDependencies", "showNotes", "showStyle", "showAdvancedForm", "taskFormClass", "advancedFormClass", "tabsConfig", "taskFormConfig", "dependencyGridConfig", "assignmentGridConfig", "advancedFormConfig", "styleFormConfig"];
...
//多语种右键项目计划启动消息
function getRightClickPlanPopMsgByLangCode() {

    var varLangCode = getcookie("LangCode");
    var varMsg;

    if (varLangCode == "zh-CN") {
        varMsg = '警告，这个是新增的计划，系统将马上自动进行刷新保存，然后你才能进行操作！';
    }
    else if (varLangCode == "zh-tw") {
        varMsg = '警告，這個是新增的計劃，系統將馬上自動進行刷新保存，然後你才能進行操作！';
    }
    else if (varLangCode == "en") {
        varMsg = 'Warning, this is a new schedule, and the system will automatically refresh and save it immediately before you can do it！';
    }
    else if (varLangCode == "de") {
        varMsg = 'Warning, this is a new schedule, and the system will automatically refresh and save it immediately before you can do it！';
    }
    else if (varLangCode == "es") {
        varMsg = 'AWarning, this is a new schedule, and the system will automatically refresh and save it immediately before you can do it！';
    }
