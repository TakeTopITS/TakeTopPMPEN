...
public partial class TakeTopSystemImportantOtherCodeRunPage : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Page.IsPostBack != true)
        {
            ...
            int intUpdateColumnRunMarkInDB = GetUpdateColumnValueCodeRunmark();
            //设置这个值，可以决定是否执行下面的代码
            ...
            if (intUpdateColumnRunMarkInDB < intUpdateColumnRunMark)
            {
                try
                {
                    ...
                    //语句执行完毕，设置标记
                    SetUpdateColumnValueCodeRunmark(intUpdateColumnRunMark);

                    ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "location.reload();", true);
                }
                catch (Exception err)
                {
                    LogClass.WriteLogFile("Error page: " + Request.Url.ToString() + "\n" + err.Message.ToString() + "\n" + err.StackTrace);
                    //ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "lalocation.reload();", true);
                }
            }

            int intUpdateModuleRunMarkInDB = GetUpdateModuleNameCodeRunMark();
            //设置这个值，可以决定是否执行下面的代码
            ...
            if (intUpdateModuleRunMarkInDB < intUpdateModuleRunMark)
            {
                ...
                {
                    //执行模组栏英文化的语句
                    ExcuteUpdateProModuleNameToEnglish();

                    //语句执行完毕，设置标记
                    SetUpdateModuleNameCodeRunMark(intUpdateColumnRunMark);
                }
                catch (Exception err)
                {
                    //LogClass.WriteLogFile("Error page: " + Request.Url.ToString() + "\n" + err.Message.ToString() + "\n" + err.StackTrace);
            ...
            ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "CloseLayer();", true);
            //Update public.t_othercoderunmark set updatecolumnvaluecoderunmark=0,updatemodulenamecoderunmark=0;
    ...
    public static void UpdateTableStatus()
    {
        ...
        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
        {
            ...
            //try
            //{
            //    strHQL = string.Format(@"ALTER TABLE {0} ALTER COLUMN {1} TYPE VARCHAR(200);", tableName, columnName);
            //    ShareClass.RunSqlCommand(strHQL);
            //}
            //catch (Exception ex)
            //{
            //    LogClass.WriteLogFile(strHQL + " " + ex.Message);
            //}
            ...
            string updateQuery = $@"
                ...
                WHERE {columnName} IN (
                    ...
                    '停用', '停用', '测试中', '测试中', '失败', '失败', '成功', '成功', '投标中', '投标中', '已立项', '已立项', '未中标');";
    ...
    public static void UpdateTableType()
    {
        ...
        WHERE c.column_name LIKE '%type%'
          AND c.table_schema = 'public'
          AND t.table_type = 'BASE TABLE'
          AND c.table_name NOT IN ('pbcatvld', 'pbcatedt',' t_rcjprojectcostfeeids','rcjprojectadjustpricelist') AND C.column_name not like '%id%';";
        DataSet ds = ShareClass.GetDataSetFromSql(strHQL, "T_Table");
    ...
    //执行模组英文化的语句
    protected void ExcuteUpdateProModuleNameToEnglish()
    {
        // 读取SQL语句文件
        string sqlFilePath = Server.MapPath("UpdateCode/UpdateModule.txt");
    ...
    //执行模组流程定义英文化的语句
    public void UpdateModulesDefinition()
    {
    ...
    // 替换字符串中的第一个出现的指定子串
    private string ReplaceFirstOccurrence(string original, string oldText, string newText)
    {
