...
public partial class TTDIYRelatedWorkFlowForm : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            strCNRelatedType = "Contract";   
            strRelatedName = GetConstractName(strRelatedID);

            strOriginRelatedType = "Contract";   
            strOriginRelateID = strRelatedID;
        ...
        {
            strCNRelatedType = "Bid";   
            strRelatedName = GetTenderName(strRelatedID);

            strOriginRelatedType = "Bid";   
            strOriginRelateID = strRelatedID;
    ...
    protected void DL_WorkFlowTemplate_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                TB_WorkFlowName.Text = LanguageHandle.GetWord("XiangMuJiHua") + strOriginRelateID + strRelatedName + strTemName;
            }
        ...
        {
            ClientScript.RegisterStartupScript(this.GetType(), "HH99H", LanguageHandle.GetWord("scriptalertJingGaoCiLiuChengMo"));
    ...
    protected void BT_ExportFormToExcel_Click(object sender, EventArgs e)
    {
        ...
        {
            ClientScript.RegisterStartupScript(this.GetType(), "", "<script>alert('" + LanguageHandle.GetWord("ShiBai") + "');</script>");
        }
    }
    ...
    protected void BT_ActiveYes_Click(object sender, EventArgs e)
    {
        ...
        if (PublishUrl == OldPublishUrl | OldPublishUrl == null)
        {
            ...
            if (intWLID <= 0)
            {
                ...
                if (intWLID == -1)
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "", "<script>alert('" + LanguageHandle.GetWord("ZZTJSBXYSJWJBNCGBCJC") + "');</script>");
                }

                if (intWLID == -2)
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "", "<script>alert('" + LanguageHandle.GetWord("ZZTJSBBNSCXGZLJC") + "');</script>");
                }

                if (intWLID == -3)
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "", "<script>alert('" + LanguageHandle.GetWord("ZZTJSBGZLMCHMSBNWKJC") + "');</script>");
                }
                ...
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "", "<script>alert('" + LanguageHandle.GetWord("ZZJGZXJZTJSJSBJC") + "');</script>");
                }
            }
            else
            {
