...
public partial class TTCreatedProject : System.Web.UI.Page
{
    string strLangCode;
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Page.IsPostBack != true)
        {
            ...
            //strHQL = string.Format(@"select C.*,COALESCE(D.TotalBL,0) PercentRea from T_Project C left join (select A.ProjectID,COALESCE(B.TotalRea,0)/CASE WHEN A.Total = 0 Then 1 END as TotalBL from (select 
            ...
            //    T_ProjectCostManage where Type='Operation' group by ProjectID) B on A.ProjectID=B.ProjectID) D on C.ProjectID=D.ProjectID where C.UserCode='{0}' and 
            //    C.Status not in ('New','Hided','Deleted','Archived') Order by C.ProjectID DESC", strUserCode); 
            ...
            strHQL = string.Format(@"WITH Cost AS (
                    ...
                    WHERE C.UserCode = '{0}'
                    AND C.Status NOT IN ('New', 'Hided', 'Deleted', 'Archived')
                    ORDER BY C.ProjectID DESC LIMIT 30;
                ", strUserCode);
            DataSet ds = ShareClass.GetDataSetFromSql(strHQL, "T_ProjectCost");
            DataGrid1.DataSource = ds;
            DataGrid1.DataBind();

            LB_QueryScope.Text = LanguageHandle.GetWord("ZZLiXiangRen").ToString().Trim() + strUserName + LanguageHandle.GetWord("StatusAll").ToString().Trim();
            LB_Sql.Text = strHQL;
    ...
    protected void DataGrid2_ItemCommand(object sender, DataGridCommandEventArgs e)
    {
        ...
        LB_Sql.Text = strHQL;
        LB_QueryScope.Text = LB_QueryScope.Text = LanguageHandle.GetWord("ZZLiXiangRen").ToString().Trim() + strUserName + LanguageHandle.GetWord("nbspnbspZhuangTai").ToString().Trim() + strStatus;
    }
    ...
    //    LB_Sql.Text = strHQL;
    //    LB_QueryScope.Text = LB_QueryScope.Text = LanguageHandle.GetWord("Project").ToString().Trim() + strProjectID + LanguageHandle.GetWord("StatusAll").ToString().Trim();
    //}
    ...
    protected void BT_ProjectIDFind_Click(object sender, EventArgs e)
    {
        ...
        if (strFindCondition == "")
        {
            //strHQL = "from Project as project where project.UserCode = " + "'" + strUserCode + "'" + " and project.Status not in ( 'Hided','Deleted','Archived') order by project.ProjectID DESC";
        ...
        {
            //strHQL = "from Project as project where project.ProjectID = " + strFindCondition + " and project.UserCode = " + "'" + strUserCode + "'" + "  and project.Status not in ( 'Hided','Deleted','Archived') order by project.ProjectID DESC";
            strHQL = "select C.*,COALESCE(D.TotalBL,0) PercentRea from T_Project C left join (select A.ProjectID,COALESCE(B.TotalRea,0)/CASE WHEN A.Total = 0 Then 1 END as TotalBL from (select " +
                "ProjectID,SUM(Total) Total from T_ProjectCostManage Where Type='Base' group by ProjectID) A left join (select ProjectID,SUM(Total) TotalRea from " +
                "T_ProjectCostManage where Type='Operation' group by ProjectID) B on A.ProjectID=B.ProjectID) D on C.ProjectID=D.ProjectID where C.UserCode='" + strUserCode + "' and " +
    ...
    protected void BT_MakeUserFind_Click(object sender, EventArgs e)
    {
        ...
        //ProjectBLL projectBLL = new ProjectBLL();
        //strHQL = "from Project as project where project.UserName like " + "'" + strFindCondition + "'" + " and project.UserCode = " + "'" + strUserCode + "'" + "  and project.Status not in ( 'Hided','Deleted','Archived') order by project.ProjectID DESC";
        //lst = projectBLL.GetAllProjects(strHQL);
