...
public partial class TTMeetingToTask : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string strMeetingID = Request.QueryString["MeetingID"];
...
HE_Operation.Language = Session["LangCode"].ToString();
        ...
        //this.Title = "会议：" + strMeetingID + meeting.Name + " 转成任务";
        ...
        if (Page.IsPostBack != true)
        {
            ...
            TB_Task.Text = meeting.Name.Trim();
            DLC_BeginDate.Text = meeting.BeginTime.ToString("yyyy-MM-dd");
            DLC_EndDate.Text = meeting.EndTime.ToString("yyyy-MM-dd");
            ...
            if (strIsMobileDevice == "YES")
            {
                HT_Operation.Text = LanguageHandle.GetWord("ZhuTi").ToString().Trim() + meeting.Name.Trim() + LanguageHandle.GetWord("brbrNeiRong").ToString().Trim() + meeting.Content.Trim();
            }
            else
            {
                HE_Operation.Text = LanguageHandle.GetWord("ZhuTi").ToString().Trim() + meeting.Name.Trim() + LanguageHandle.GetWord("brbrNeiRong").ToString().Trim() + meeting.Content.Trim();
            }
    ...
    protected void LoadProjectTask()
    {
        ...
        string strHQL = "from ProjectTask as projectTask where projectTask.MeetingID = " + strMeetingID + " Order by projectTask.TaskID ASC";
        ProjectTask projectTask = new ProjectTask();
        ProjectTaskBLL projectTaskBLL = new ProjectTaskBLL();
        IList lst = projectTaskBLL.GetAllProjectTasks(strHQL);
        DataGrid1.DataSource = lst;
