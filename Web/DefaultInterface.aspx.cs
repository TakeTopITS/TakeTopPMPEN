...
public partial class DefaultInterface : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string strThirdPartLoginID;
        strThirdPartLoginID = Request.QueryString["code"];
        if (string.IsNullOrEmpty(strThirdPartLoginID))
        {
    ...
    protected void LB_Login_Click(object sender, EventArgs e)
    {
        ...
        string strVerificationCode, strSMSVerification, strSMSCode, strCheckCode, strVersionType;
        ...
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                //第一次登录时，把用户的第三方LOGINID写入人员档案表
                string strThirdPartLoginID = LB_ThirdPartLoginID.Text.Trim();
                if (strThirdPartLoginID != "")
                {
                    strHQL = "Update T_ProjectMember Set ThirdPartLoginID = " + "'" + strThirdPartLoginID + "'" + " Where UserCode = " + "'" + strUserCode + "'";
    ...
    protected void LoginByThirdPartLoginID(string strThirdPartLoginID)
    {
        string strUserCode, strUserName;
        string strUserType, strMDIStyle, strMDIPageName, strMDIMobilePageName, strThirdPartPageNme, strThirdPartMobilePageName;
        string strUserHostAddress, strAllowDevice, strLangCode;
        ...
        try
        {
            ...
            strHQL = "Select UserCode,UserName,UserType,AllowDevice,MDIStyle,CssDirectory From T_ProjectMember Where ThirdPartLoginID = " + "'" + strThirdPartLoginID + "'";
            strHQL += " and UserCode in (Select UserCode From T_SystemActiveUser Where WebUser = 'YES')";
            DataSet ds = ShareClass.GetDataSetFromSql(strHQL, "T_ProjectMember");

            if (ds.Tables[0].Rows.Count > 0)
            {
                strUserCode = ds.Tables[0].Rows[0]["UserCode"].ToString();
                strUserName = ds.Tables[0].Rows[0]["UserName"].ToString();
                strUserType = ds.Tables[0].Rows[0]["UserType"].ToString();
                strAllowDevice = ds.Tables[0].Rows[0]["AllowDevice"].ToString();
                strLangCode = ds.Tables[0].Rows[0]["LangCode"].ToString();
    ...
    protected int CheckUserThirdPartLoginID(string strThirdPartLoginID)
    {
        string strHQL;

        strHQL = "Select * From T_ProjectMember Where ThirdPartLoginID = " + "'" + strThirdPartLoginID + "'";
        DataSet ds = ShareClass.GetDataSetFromSql(strHQL, "T_ProjectMember");
    ...
    protected void ddlLangSwitcher_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        Response.Redirect("DefaultInterface.aspx");
    }
